...
//            std::cout << i << std::endl;
//        };
//        std::thread t(f);   //lanza thread
//        threads.push_back(std::move(t)); //almacena referencia 
//    }
//    for (auto& t : threads) {
//        t.join();
//    }
...
//        {
//            std::lock_guard<std::mutex> lock(print_mutex); //antes de acceder al recurso lo vamos a bloquear
//            std::cout << i << std::endl;
//        }; // liberamos mutex cuando sale del scope de la función 
//        std::thread t(f);
...
//{
//    std::vector<std::thread> threads;
//    std::atomic_int counter{ 0 }; // = std::atomic<int> counter
//    std::mutex print_mutex;
//    for (int i = 0; i < 10; ++i) {
...
//        }
//    );
//    t.detach(); //no espera a que acabe el thread para seguir ejecutando
//    for (int i = 0; i < 10; ++i) {
...
//    auto res = pool.map<std::function<int(int)>, int, int> //<int(int)> recibe y devuelve entero
//    (
//        [](int i)
//        {
